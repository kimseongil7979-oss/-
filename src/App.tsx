...
import { Phone, Check, Clock, ShieldCheck, Mail, MapPin, FileText } from "lucide-react";
...
export default function App() {
  ...
  const scrollToInquiry = () => {
    setIsAdminOpen(false); // return to landing if in admin view
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans antialiased text-slate-800">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-slate-200 py-2.5 px-4 shadow-2xl flex items-center justify-between gap-3 select-none">
        <button
          onClick={scrollToInquiry}
          className="flex-1 flex items-center justify-center gap-1.5 bg-slate-100 hover:bg-slate-200 active:scale-95 transition-all text-slate-800 font-extrabold text-xs py-3 rounded-xl border border-slate-200"
        >
        ...
        <a
          href="tel:010-4122-7980"
          className="flex-[2] flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 active:scale-95 transition-all text-white font-black text-xs py-3 rounded-xl shadow-md"
        >
          <Phone className="w-4 h-4 text-white animate-pulse" />
          <span>전화로 즉시 문의</span>
        </a>
